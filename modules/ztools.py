...
from modules.shuffle import nshuffle
...
def hamming_parity(fname_in, fname_out, power, len_nes=0):
    gen_m = matrix.gen_matrix(power)[0]
    ...
    for i in range(len_nes // len_block_data):
        block_data = np.matrix([int(j) for j in list(ba_key[i * len_block_data:(i + 1) * len_block_data])])
        if debug: print(block_data)

        block_parity = (np.dot(gen_m, block_data.transpose()) % 2).transpose().tolist()[0]
        block_parity.append((sum(block_parity) + sum(ba_key[i * len_block_data:(i + 1) * len_block_data])) % 2)
...
# len_nes in blocks
def hamming_correct(fname_key, fname_parity, fname_kout, fname_bad, power, len_nes=0, len_nes_par=0, drop_bad=False):
    syn_m = matrix.gen_matrix(power)[1]
    len_block_data = 2 ** power - 1 - power
    ...
    if not len_nes: len_nes = len(ba_key) // len_block_data
    ba_result = bitarray()
    bad_blocks = []
    ...
    for i in range(len_nes):
        ...
        if debug: print(block_data, block_parity)
        ...
        for j in range(len(block_merged)):
            if len(block_data):
                if block_merged[j] == -1:
                    ...
                    block_data.pop(0)
        if debug: print(f'#{i} - block: {block_merged}')
        block_syndrome = (np.dot(np.matrix(block_merged), syn_m.transpose()) % 2).tolist()[0]
        if debug and sum(block_syndrome):
            print(f'#{i} - block: {block_merged}')
            print(f'syn: {block_syndrome}')
        if debug: print(f'syn: {block_syndrome}')
        
        if drop_bad and (block_syndrome[0] != (sum(block_syndrome) > 0)):
            if debug: print(f'Bad block!')
            bad_blocks.append(i)
            continue
...
def shuffle(fname_in, fname_out, len_block, len_nes):
    ...
    if not len_nes: len_nes = len(ba_in) // len_block

    for i in range(len_nes):
        ba_in[i * len_block:(i + 1) * len_block] = bitarray(nshuffle.gen_shuffle(ba_in[i * len_block:(i + 1) * len_block]))
        # print(f'Shuffled #{i}!')
